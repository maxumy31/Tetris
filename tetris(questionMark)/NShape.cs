...
namespace tetris_questionMark_
{
    internal class NShape : Shape
    {
        ...
        public override int[,] rotateShape(int[,] coords, int[,] occupiedGrid)
        {
            ...
            if (turn == 1 && coords[2,0] < 9 && Utils.CheckCollision(coords, occupiedGrid))
            {
                coords[3, x] += 1;
                coords[3, y] += 1;
            ...
            if (turn == 3 && coords[2,0] > 0 && Utils.CheckCollision(coords, occupiedGrid))
            {

...
namespace tetris_questionMark_
{
    internal class JShape:Shape
    {
        ...
        public override int[,] rotateShape(int[,] coords, int[,] occupiedGrid)
        {
            ...
            if (turn == 1 && Utils.CheckIfCloseToLeftSide(coords) && Utils.CheckCollision(coords, occupiedGrid))
            {
                coords[3, x] += 0;
                coords[3, y] -= 2;
            ...
            if (turn == 3 && Utils.CheckIfCloseToLeftSide(coords) && Utils.CheckCollision(coords,occupiedGrid))
            {

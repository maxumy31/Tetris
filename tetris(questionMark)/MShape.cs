...
{
    internal class MShape : Shape
    {
        public override int[,] createShape()
        {
            Random random = new Random();
            coords[0, 0] = random.Next(1, 10);
            coords[0, 1] = 0;
        ...
        public override int[,] rotateShape(int[,] coords, int[,] occupiedGrid)
        {
            ...
            if (turn == 1 && coords[2, 0] < 9 && coords[2, 0] > 0 && Utils.CheckCollision(coords, occupiedGrid))
            {
                coords[3, x] += 1;
                coords[3, y] += 1;
            ...
            if (turn == 3 && coords[2, 0] < 9 && coords[2, 0] > 0 && Utils.CheckCollision(coords, occupiedGrid))
            {

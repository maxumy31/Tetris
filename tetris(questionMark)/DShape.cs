...
namespace tetris_questionMark_
{
    internal class DShape : Shape
    {
        ...
        public override int[,] rotateShape(int[,] coords, int[,] occupiedGrid)
        {
            ...
            if (turn == 2 && occupiedGrid[coords[1, 0], coords[1,1]+1] != 1 && Utils.CheckCollision(coords, occupiedGrid))
            {
                coords[0, x] += 1;
                coords[0, y] -= 1;
            ...
            if (turn == 3 && coords[1,0] < 9 && Utils.CheckCollision(coords, occupiedGrid))
            {
